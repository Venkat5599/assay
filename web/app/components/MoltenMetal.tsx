...
/**
 * Animated fluid field for the hero plane.
 *
 * It is decorative and sits behind the content, never gating it: if WebGL is
 * unavailable, the shader simply never mounts and the flare plane stays flat.
 * The loop is suspended when the element scrolls out of view or the tab is
 * hidden, and it never starts at all under prefers-reduced-motion.
 */
...
export default function MoltenMetal({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL2. The plane stays flat; nothing else changes.
      return;
    }
    ...
    // The canvas is pointer-transparent, so track on the window instead.
    ...
    const start = () => {
      if (inView && pageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
    const stop = () => {
      if (raf !== 0) {
