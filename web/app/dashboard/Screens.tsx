...
export function CaseView({ops, id, go}: {ops: Ops; id: bigint; go: (s: string, i?: bigint) => void}) {
  const x = ops.positions.find((p) => p.id === id);
  if (!x) return <Empty>Asset #{id.toString()} is not registered on this deployment.</Empty>;

  const book = ops.bids.filter((b) => b.assetId === id);
  const events = ops.audit.filter((a) => a.detail.includes(`asset #${id}`));

  return (
    <>
      <div className="metrics">
        <Metric label="FACE VALUE" value={usd(x.face)} note={`due in ${x.days} days`} />
        <Metric label="FIRM FLOOR" value={usd(x.floor)} note={x.underwriter === ZERO ? "unpriced" : bookOf(x.underwriter)} />
        <Metric label="OUTSTANDING" value={usd(x.debt)} note={`${x.utilisation.toFixed(0)}% of headroom`} />
        <Metric label="COVERAGE" value={x.debt > 0n ? `${x.coverage.toFixed(2)}x` : "n/a"}
                tone={x.debt > 0n && x.coverage < COVERAGE_THRESHOLD ? "bad" : "ok"}
                note={`policy ${COVERAGE_THRESHOLD}x`} />
        <Metric label="STATUS" value={x.status} tone={toneFor(x.status)} />
      </div>

      <div className="cols">
        <section>
          <Head title="Credit file" right={`ASSET #${id}`} />
          <dl className="kv">
            <div><dt>Obligor</dt><dd className="mono">{x.obligor}</dd></div>
            <div><dt>Document hash</dt><dd className="mono">{x.docHash}</dd></div>
            <div><dt>Registered to</dt><dd className="mono">{x.owner}</dd></div>
            <div><dt>Face value</dt><dd>{usd(x.face)}</dd></div>
            <div><dt>Settles</dt><dd>{new Date(Number(x.dueDate) * 1000).toISOString().slice(0, 10)}</dd></div>
            <div><dt>Incumbent</dt><dd className="mono">{x.underwriter === ZERO ? "none" : x.underwriter}</dd></div>
            <div><dt>Escrow held</dt><dd>{usd(x.escrow)}</dd></div>
            <div><dt>Premium reserve</dt><dd>{usd(x.premiumReserve)}</dd></div>
          </dl>

          <Head title="Settlement flow" right={x.defaulted ? "CALLABLE" : "NOT CALLABLE"} />
          <pre className="plate">
{`  RECEIVABLE #${x.id}
       |
       v
  UNDERWRITER  <-- pays ${usd(x.floor)} from escrow
       |
       +---- ${usd(x.debt > x.floor ? x.floor : x.debt)} --> LENDER  (made whole first)
       |
       +---- ${x.floor > x.debt ? usd(x.floor - x.debt) : "0"} --> CARRIER (surplus)

  ESCROW VERIFIED       ${x.escrow >= x.floor ? "yes" : "no"}
  COLLATERAL ESCROWED   ${x.open ? "yes" : "no"}
  CALLABLE              ${x.defaulted ? "yes" : "no"}`}
          </pre>
        </section>

        <section className="stack">
          {!x.open && <OpenSlot assetId={x.id} onDone={ops.refresh} />}
          {x.open && (
            <BorrowRepay assetId={x.id} drawable={x.drawable} debt={x.debt} onDone={ops.refresh} />
          )}
          {x.open && (
            <PlaceBid assetId={x.id} currentFloor={x.underwriter !== ZERO ? x.floor : undefined} onDone={ops.refresh} />
          )}
        </section>
      </div>

      <Head title="Bid history" right={`${book.length} BIDS`} />
      {book.length === 0 ? (
        <Empty>No bids on this receivable.</Empty>
      ) : (
        <div className="tablewrap">
          <table className="tbl">
            <thead><tr><th>DESK</th><th className="t-num">FLOOR</th><th className="t-num">BLOCK</th><th>TX</th></tr></thead>
            <tbody>
              {[...book].reverse().map((b) => (
                <tr key={b.hash}>
                  <td className="t-id">{bookOf(b.underwriter)}</td>
                  <td className="t-num">{usd(b.floor)}</td>
                  <td className="t-num">{b.block.toString()}</td>
                  <td><a href={explorerTx(b.hash)} target="_blank" rel="noreferrer">view</a></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <button className="linkish" onClick={() => go("portfolio")}>back to portfolio</button>
    </>
  );
}
